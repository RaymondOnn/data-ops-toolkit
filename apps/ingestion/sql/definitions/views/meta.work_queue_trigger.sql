...
SELECT
    ...
    -- FIX: Replaced generateUUIDv4() with a deterministic hash of the slot.
    -- This ensures the same RUN_ID is generated even if the MV refreshes multiple times.
    concat(
        formatDateTime(NEXT_RUN_TS_LC, '%Y%m%d-%H%i%s'),
        '-',
        substring(lower(hex(MD5(concat(JOB_ID, DATASET_ID, toString(NEXT_RUN_TS_LC))))), 1, 8)
    ) AS RUN_ID
    , JOB_ID
    , DATASET_ID
