...
SELECT
    ...
    , now64(3, 'Asia/Singapore') AS LAST_UPDATED_AT_TS_LC
    , cast(
        if(S.IS_SNAPSHOT = 1 AND now64(3, 'Asia/Singapore') > cron_next(if(empty(S.CRON_EXPR), '0 0 * * *', S.CRON_EXPR), NEXT_RUN_TS_LC), 
            'EXPIRED', 
            'PENDING'
