...
class JobStep(ABC):
    ...
    def prepare_stage(self, job: "Job") -> None:
        ...
        target_dir = Path(JOB_STEPS_BASE_DIR).expanduser() / self.name / job.id / job.run_id
        ...
        if current_dir != target_dir:
            ...
            if current_dir.exists():
                ...
                LOG.info(f"[{job.worker_id}] Moved folder: {current_dir.name} -> {self.name}")
            else:
...
class AuditStep(JobStep):
    ...
    def _run_internal_checks(self, job: "Job", write_meta: WritePayload) -> dict[str, Any]:
        ...
        if write_meta.rows_affected == 0:
            failures.append({"check": "row_count_not_zero", "message": "No rows were loaded to staging."})
...
class PublishStep(JobStep):
    ...
    def execute(self, job: "Job") -> str:
        ...
        try:
            ...
            strategy.promote(
                client=client, 
                staging_info={"staging_artifact": write_meta.staging_artifact},
                target=write_meta.target_identifier
            )
...
class CompleteStep(JobStep):
    ...
    def execute(self, job: "Job") -> str:
        ...
        try:
            ...
            retention_days = getattr(job.job_config, "retention_days", 2555) # 7 years default
            expiry_date = (end_ts + timedelta(days=retention_days)).date().isoformat()
