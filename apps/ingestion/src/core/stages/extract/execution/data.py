...
class DataExtractor(Extractor[T_Source], Generic[T_Source]):
    ...
    def _ray_generator(
        ...
        # 1. Put context into Ray shared memory once
        context_ref = ray.put(context)

        # 2. Keep dataset items lightweight (units are str for DB, dict for Files)
        ...
        def worker_task(batch: dict[str, list[Any]], ctx_ref: ray.ObjectRef) -> Any:
            ...
            from src.core.stages.extract.execution.data import flatten_df
            from src.core.stages.extract.execution.schema import (
                apply_schema_contract,
            )
            from src.services.factory import ServiceFactory
            from src.services.health.monitor import ServiceMonitor

            ctx: ExtractContext = ray.get(ctx_ref)
            unit = batch["unit"][0]
        ...
        global_limit = (
            context.sql_context.main.limit if context.sql_context.main else None
        )
        accumulated_rows = 0
...
class FileExtractor(DataExtractor[FileSource]):
    ...
    def _get_work_units(
        self, source: FileSource, context: ExtractContext
    ) -> Sequence[Any]:
