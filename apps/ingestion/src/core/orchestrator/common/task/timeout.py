...
from src.core.stages.types import ALL_STAGES
...
class TimeoutState(msgspec.Struct):
    ...
    def get_deadline(self) -> float:
        now = time.monotonic()
        stage_deadline = now + self.stage_timeout
    ...
    def complete_stage(self, duration: float) -> None:
        self.budget_used += duration
...
class TimeoutMonitor:
    ...
    def _pre_fetch_stage_timeouts(self, job_id: str, dataset_id: str) -> None:
        ...
        try:
            # 1. Fetch raw query metrics from repository
            metrics = self.meta.fetch_p95_stage_durations(job_id, dataset_id, stages)

            # 2. Apply business rules / calculations
            for stage, data in metrics.items():
                if data["sample_count"] > 10:
                    timeout = min(data["p95"] * 1.5, STAGE_CAPS.get(stage, 14400))
                    timeouts[stage] = timeout
                    history_count += 1
                else:
                    timeouts[stage] = STAGE_DEFAULTS.get(stage, 1800)

            # 3. Apply defaults for remaining unreturned stages
            for stage in stages:
        ...
        # 4. Cache state stays in TimeoutMonitor
...
class TimeoutContext:
    ...
    def __init__(
        ...
        if self.state:
            ...
            # Calculate deadline
            self.deadline = self.state.get_deadline()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.state:
            ...
            self.state.complete_stage(duration)
    ...
    def check(self):
        ...
        if time.monotonic() > self.deadline:
            raise TimeoutError(
                f"Step '{self.step_id}' exceeded timeout deadline "
                f"(timeout: {self.state.stage_timeout}s, "
                f"elapsed: {time.monotonic() - self.start_time:.1f}s)"
            )
