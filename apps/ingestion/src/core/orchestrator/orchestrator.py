...
class Orchestrator:
    ...
    def _provision_pending_run(self, record: dict[str, Any]) -> None:
        """
        Adopts a run generated by the ClickHouse trigger view.
        Creates physical workspace and moves it to the Engine.
        """
        job_id = record["JOB_ID"]
        dataset_id = record["DATASET_ID"]
        ...
        log = LOG.bind(job_id=job_id, run_id=run_id)

        # 1. Build context (The instruction set for the worker)
        task_ctxs = self.builder.build(
            job_id=job_id, dataset_id=dataset_id, run_date_str=run_date
        )

        for ctx in task_ctxs:
            identifier = self.exec_ctx.get_task_identifier(job_id, dataset_id, run_date)

            # 2. Physically provision the workspace
            # This creates the folder and config.json using the RUN_ID from ClickHouse
            task = Task(
                run_id=run_id,
