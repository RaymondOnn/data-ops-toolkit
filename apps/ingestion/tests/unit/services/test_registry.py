...
import pytest
from apps.ingestion.src.services.registry import ServiceRegistry, protect_service
from libs.resilience.circuit_breaker import CircuitBreaker, CircuitBreakerTripped


class TestServiceRegistry:
    """Unit tests for the ServiceRegistry global state manager."""

    @pytest.fixture(autouse=True)
    def setup_registry(self, tmp_path):
        """
        GIVEN a fresh test environment
        THEN configure the ServiceRegistry with a temporary workspace
        WHEN each test starts
        """
        # Reset class variables before each test
        ServiceRegistry._cache = None
        ServiceRegistry._signal_path = None

        workspace = tmp_path / "registry_test"
        workspace.mkdir()
        (workspace / "signals").mkdir()

        ServiceRegistry.configure(workspace)
        yield
        ServiceRegistry._cache = None
        ServiceRegistry._signal_path = None
    ...
    def test_status_update_and_retrieval(self):
        ...
        """
        ServiceRegistry.update_status("ClickHouse", "OPEN")
        assert ServiceRegistry.get_status("ClickHouse") == "OPEN"
        assert ServiceRegistry.is_healthy("ClickHouse") is False
    ...
    def test_signal_file_management(self):
        ...
        WHEN update_status is called
        """
        ServiceRegistry.update_status("Postgres", "OPEN")
        signal_file = ServiceRegistry._signal_path / "postgres.outage"
        assert signal_file.exists()

        ServiceRegistry.update_status("Postgres", "CLOSED")
    ...
    def test_increment_failure_with_windowing(self):
        ...
        """
        # First failure
        count = ServiceRegistry.increment_failure("API", window_seconds=5)
        assert count == 1
    ...
    def test_circuit_breaker_tripping_threshold(self):
        ...
        ServiceRegistry.increment_failure("DB", window_seconds=0)
        ServiceRegistry.increment_failure("DB", window_seconds=0)
        ServiceRegistry.increment_failure("DB", window_seconds=0)

        assert ServiceRegistry.get_status("DB") == "OPEN"
        assert ServiceRegistry.get_failure_count("DB") == 3
    ...
    def test_reset_clears_metrics(self):
        ...
        """
        ServiceRegistry.update_status("S3", "OPEN")
        ServiceRegistry.increment_failure("S3", window_seconds=0)

        ServiceRegistry.reset("S3")

        assert ServiceRegistry.get_status("S3") == "CLOSED"
        assert ServiceRegistry.get_failure_count("S3") == 0
        assert ServiceRegistry.get_last_failure_time("S3") == 0.0
    ...
    def test_probe_success_triggers_reset(self):
        ...
        """
        ServiceRegistry.update_status("Legacy", "OPEN")

        success = ServiceRegistry.probe("Legacy", lambda: True)

        assert success is True
        assert ServiceRegistry.is_healthy("Legacy") is True
...
class TestProtectServiceDecorator:
    ...
    class MockService:
        ...
        @protect_service(CircuitBreaker(failure_threshold=1))
    ...
    def setup_registry(self, tmp_path):
        ServiceRegistry._cache = None
        ServiceRegistry.configure(tmp_path)
    ...
    def test_decorator_syncs_with_registry_on_failure(self):
        ...
        assert ServiceRegistry.get_status("MockSvc") == "OPEN"

    def test_decorator_blocks_when_registry_is_open(self):
        """
        GIVEN the global registry status is 'OPEN' for a service
        THEN calling the decorated method should raise CircuitBreakerTripped
        WHEN the method is invoked
        """
        ServiceRegistry.update_status("MockSvc", "OPEN")
        ServiceRegistry.set_last_failure_time("MockSvc", time.time())
