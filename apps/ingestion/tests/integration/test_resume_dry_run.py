...
def test_janitor_recovery_dry_run(runtime, tmp_path, caplog):
    ...
    task.workspace.create_stage_marker("transform", transform_data_path)
    
    task.update_manifest({
        "extract": {"file_count": 1}, 
        "bitmask": StageName.EXTRACT.bitmask | StageName.TRANSFORM.bitmask,
        "current_stage": StageName.TRANSFORM.label,
    ...
    with caplog.at_level("INFO"):
        runtime.orchestrator.janitor.recover_task_by_path(Path(failed_path), dry_run=True)
    ...
    assert (Path(failed_path) / StageName.EXTRACT.label).exists(), "Extract marker should still exist"
    assert (Path(failed_path) / StageName.TRANSFORM.label).exists(), "Transform marker should still exist"
    
    # Verify manifest on disk is unchanged
    reloaded_task = Task.from_folder(Path(failed_path), runtime.exec_ctx)
    assert reloaded_task.manifest.status == ExecutionStatus.FAILED
    assert reloaded_task.manifest.current_stage == StageName.TRANSFORM.label
    assert reloaded_task.manifest.bitmask == (StageName.EXTRACT.bitmask | StageName.TRANSFORM.bitmask)
