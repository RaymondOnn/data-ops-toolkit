...
class CASArchiveMixin:
        ...
        def archive_to_cas(
            ...
            if not self.fs.exists(vault_path):
                ...
                LOG.info("CAS: Content already exists in vault, skipping upload.", extra={"hash": file_hash})
        ...
        def retrieve_file(self, job_id: str, date_str: str) -> str:
            ...
            if not self.fs.exists(physical_path):
                raise FileNotFoundError(f"Manifest exists, but physical file is missing: {physical_path}")
