...
from .archive import ArchiveSkill
from .base import FileSkill

LOG = logging.getLogger(__name__)


@FileSkill.register(key="data")
class FileReader(FileSkill):
    ...
    def archive(self) -> ArchiveSkill:
        """Peer skill instantiation using the shared filesystem client."""
        return ArchiveSkill(self.fs)
    ...
    def extract(
        ...
        **kwargs: Any,
        ...
        if archive_files:
            ...
            with self.archive.open(base_archive, temp_dir=temp_folder) as archive_sess:
                ready_files: list[str] = []
                for path in formatted_paths:
                    if "!!" in path:
                        _, inner_file = path.split("!!", 1)
                        ready_files.append(archive_sess.extract_member(inner_file))
                    else:
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        directly to the appropriate FormatHandler.
        """
        ...
        handler = FormatHandler.create(fmt, self.fs.options)
    ...
    def format_archive_path(
        ...
        if not self.archive.is_supported(resolved):
            return path
