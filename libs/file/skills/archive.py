...
class ArchiveSession:
    ...
    def list_contents(self) -> list[dict[str, Any]]:
        ...
        with self.fs.open(self.archive_path, "rb") as f:
            if ArchiveSkill.is_zip(self.archive_path):
...
@FileSkill.register(key="archive")
class ArchiveSkill(FileSkill):
    """FileSkill providing archive virtualization and context extraction."""
    ...
    def open(
        ...
        """Factory method to instantiate an ArchiveSession context manager."""
        return ArchiveSession(self.fs, archive_path, temp_dir=temp_dir)
